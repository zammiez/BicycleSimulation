...
inline void ClampAngleDeg(double& angle)
{
	while (angle > 180.0f)
	{
		angle -= 360.0f;
...
inline double AngleDiff(double angle0, double angle1)
{
	double value = angle0 - angle1;
	ClampAngle(value);
	return value;
}

inline double Lerp(double v0, double v1, double fPerc)
{
